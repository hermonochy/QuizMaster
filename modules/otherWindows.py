...
def about(BACKGROUND_COLOUR, BUTTON_COLOUR, BLACK):
    ...
    description = """
     QuizMaster is an engaging, interactive game designed to challenge your knowledge and cognitive skills across a variety of topics. Whether you're a trivia buff or simply looking to test your memory, QuizMaster offers a platform to sharpen your intellect while having fun. The game's structure incorporates different modes, allowing players to select their preferred play style, ensuring each session is both enjoyable and educational.

    QuizMaster features a variety of game modes tailored for different levels of difficulty and gameplay preferences. From the classic countdown quiz to speed runs that challenge your quick thinking, each mode is designed to provide a unique experience. Players can also enjoy a preferences window that allows customization of song choices, volume, and background colors, making the game truly your own.

    Join the QuizMaster community today, and contribute to a growing repository of quizzes while testing your knowledge in an entertaining format. Remember, learning can be a fun and exciting journey with QuizMaster!
    """
    ...
    while running:
        ...
        button_website = Button("Our website", (SCREEN_WIDTH // 2 - 125, SCREEN_HEIGHT // 2 + 50), 200, 30)
        display_message("About QuizMaster", SCREEN_HEIGHT // 8, 75, BLACK)
        display_message(description, SCREEN_HEIGHT // 5, 30, BLACK)
        display_message("For more information, please vist", SCREEN_HEIGHT // 2, 30, BLACK)
        button_website.draw(screen, BUTTON_COLOUR)
...
def Licenses(BACKGROUND_COLOUR, BUTTON_COLOUR, BLACK):
    ...
    while running:
        ...
        button_GPL.draw(screen, BUTTON_COLOUR)
        button_CC.draw(screen, BUTTON_COLOUR)
        button_go_back.draw(screen, BUTTON_COLOUR)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == QUIT:
